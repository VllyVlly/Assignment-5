...
void initialize_graph(graph_args* args,
    ...
    args->compact_offsets.clear();
    args->compact_offsets.resize(node_count + 1);
    args->compact_neighbors.clear();
    args->compact_neighbors.resize(node_count * static_cast<std::size_t>(avg_degree));

    args->graph.n = static_cast<int>(node_count);
    args->graph.nodes = args->nodes.data();
    args->graph.offsets = args->compact_offsets.data();
    args->graph.neighbors = args->compact_neighbors.data();
    ...
    for (std::size_t u = 0; u < node_count; ++u) {
        ...
        args->compact_offsets[u] = static_cast<int>(edge_pos);

        for (int k = 0; k < avg_degree; ++k) {
            neighbors.push_back(dist(gen));
            args->compact_neighbors[edge_pos + static_cast<std::size_t>(k)] =
                neighbors.back();
        }
    ...
    args->compact_offsets[node_count] = static_cast<int>(edge_pos);
...
void stu_graph(std::uint64_t& out, const Graph& graph) {
    std::uint64_t checksum = 0;
    const int* const offsets = graph.offsets;
    const int* const neighbors = graph.neighbors;

    for (int u = 0; u < graph.n; ++u) {
        const int begin = offsets[u];
        const int end = offsets[u + 1];
        for (int idx = begin; idx < end; ++idx) {
            checksum += static_cast<std::uint64_t>(neighbors[idx]);
        }
    }
